...
import { X, UploadCloud, History } from "lucide-react";
import imageCompression from "browser-image-compression";
import { toast } from "sonner";
import { submitUndertimeRequest } from "../../actions/requests.actions";
...
export function FileUTModal({ isOpen, onClose }: ModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await submitUndertimeRequest(formData);
      toast.success("Undertime Request Submitted!", {
          ...
          >
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-black text-zinc-950 dark:text-zinc-50 tracking-tight">File Undertime</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Actual Timeout</label>
                  <input type="time" name="actual_timeout" required className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-950 dark:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Reason</label>
                <textarea name="reason" required rows={3} placeholder="Provide reason for leaving early..." className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-950 dark:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"></textarea>
              </div>
              ...
              <button disabled={loading} type="submit" className="mt-4 w-full bg-zinc-950 dark:bg-zinc-50 text-white dark:text-zinc-950 font-black uppercase tracking-widest text-sm py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-primary hover:text-white transition-all disabled:opacity-50">
                {loading ? "Submitting..." : "Submit Undertime"}
              </button>
            </form>
          </motion.div>
        </React.Fragment>
